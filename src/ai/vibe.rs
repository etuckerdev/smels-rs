...
use crate::{ErrorInfo, AnalysisResult};
...
fn get_learning_objectives(_analysis: &AnalysisResult, vibe: &VibeMode) -> String {
    ...
    match vibe.learning_focus {
        LearningFocus::ConceptualUnderstanding => {
            ...
            objectives.push_str("3. The mental model needed to reason about this class of problems\n");
        },
        ...
        LearningFocus::SystemDesign => {
            objectives.push_str("1. How this error fits into larger system architecture concerns\n");
...
fn get_teaching_instructions(vibe: &VibeMode) -> String {
    ...
    match vibe.teaching_style {
        TeachingStyle::SocraticMethod => {
            instructions.push_str("Guide the student through discovery by asking questions like:\n");
...
pub fn handle_vibe_mode(
    ...
    format!(
        ...
        vibe.teaching_style,
        vibe.skill_level,
        if vibe.include_deep_dive { "Enabled 🔥" } else { "Disabled" },
        prompt
    )
}
...
fn parse_vibe_args(args: &str) -> VibeMode {
    ...
    // Learning focus
    if args.contains("--concepts") { vibe.learning_focus = LearningFocus::ConceptualUnderstanding; }
    else if args.contains("--patterns") { vibe.learning_focus = LearningFocus::PracticalPatterns; }
    else if args.contains("--debugging") { vibe.learning_focus = LearningFocus::DebuggingSkills; }
    else if args.contains("--practices") { vibe.learning_focus = LearningFocus::BestPractices; }
    else if args.contains("--fundamentals") { vibe.learning_focus = LearningFocus::LanguageFundamentals; }
    else if args.contains("--design") { vibe.learning_focus = LearningFocus::SystemDesign; }
    
    // Teaching style
    if args.contains("--socratic") { vibe.teaching_style = TeachingStyle::SocraticMethod; }
    else if args.contains("--building-blocks") { vibe.teaching_style = TeachingStyle::BuildingBlocks; }
    else if args.contains("--stories") { vibe.teaching_style = TeachingStyle::RealWorldContext; }
    else if args.contains("--compare") { vibe.teaching_style = TeachingStyle::CompareContrast; }
    else if args.contains("--practice") { vibe.teaching_style = TeachingStyle::HandsOnPractice; }
    
    // Skill level
    if args.contains("--beginner") { vibe.skill_level = SkillLevel::Beginner; }
    else if args.contains("--intermediate") { vibe.skill_level = SkillLevel::Intermediate; }
    else if args.contains("--advanced") { vibe.skill_level = SkillLevel::Advanced; }
    else if args.contains("--expert") { vibe.skill_level = SkillLevel::Expert; }
    
    // Options
    if args.contains("--deep-dive") { vibe.include_deep_dive = true; }

...
lazy_static! {
    static ref PANIC_PATTERNS: HashMap<&'static str, &'static str> = {
        ...
        m.insert(r"called `Option::unwrap\(\)` on a `None` value", "Option::unwrap on None");
        m.insert(r"called `Result::unwrap\(\)` on an `Err` value", "Result::unwrap on Err");
        m.insert(r"index out of bounds", "Array index out of bounds");
        m.insert(r"attempt to divide by zero", "Division by zero");
        m.insert(r"attempt to subtract with overflow", "Integer overflow/underflow");
    ...
    static ref COMPILE_PATTERNS: HashMap<&'static str, &'static str> = {
        let mut m = HashMap::new();
        m.insert(r"cannot find (?:function|struct|enum|trait|type|macro) `([^`]+)`", "Undefined identifier");
        m.insert(r"mismatched types", "Type mismatch");
        m.insert(r"borrowed value does not live long enough", "Lifetime/borrow checker error");
        m.insert(r"cannot borrow .* as mutable", "Borrow checker violation");
        m.insert(r"use of moved value", "Use after move");
        m.insert(r"this function takes \d+ argument", "Incorrect function arguments");
        m.insert(r"field `([^`]+)` of struct .* is private", "Private field access");
    ...
    static ref FRAME_NUMBER_RE: Regex = Regex::new(
        r"^\s*(\d+):\s+(.+)"
    ).unwrap();
    
    static ref FUNC_AT_LOC_RE: Regex = Regex::new(
        r"(.+?)\s+at\s+(.+)"
    ).unwrap();
}

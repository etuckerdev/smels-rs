...
impl CppParser {
    fn parse_compilation_errors(&self, input: &str) -> Vec<ErrorInfo> {
        ...
        for (pattern, description) in CPP_ERROR_PATTERNS.iter() {
            ...
            for caps in re.captures_iter(input) {
                let location = self.extract_location_from_context(input, caps.get(0).unwrap().start());
    ...
    fn parse_runtime_errors(&self, input: &str) -> Vec<ErrorInfo> {
        ...
        // Exception handling
        let exception_re = Regex::new(r"terminate called after throwing an instance of '([^']+)'").unwrap();
        ...
        if input.contains("CMake Error") {
            if input.contains("Could NOT find") {
                ...
                if let Some(caps) = find_re.captures(input) {
                    errors.push(ErrorInfo {
                        message: format!("CMake dependency not found: {}", caps.get(1).unwrap().as_str()),
                        location: None,
                        language: "cpp".to_string(),
                    });
                }
        ...
        if !input.contains("error:") {
            for (pattern, description) in CPP_WARNING_PATTERNS.iter() {
                ...
                for caps in re.captures_iter(input) {
                    let location = self.extract_location_from_context(input, caps.get(0).unwrap().start());
                    let message = if caps.len() > 1 {
                        format!("Warning - {}: {}", description, caps.get(1).unwrap().as_str())
                    } else {
    ...
    fn parse_generic_cpp_error(&self, input: &str) -> Option<ErrorInfo> {
        ...
        let cpp_indicators = [".cpp:", ".cc:", ".cxx:", ".hpp:", ".h:", "g++", "gcc", "clang++", "clang", "#include"];
        let has_cpp_content = cpp_indicators.iter().any(|indicator| input.contains(indicator));
        
        if has_cpp_content && (input.contains("error") || input.contains("failed") || input.contains("fatal")) {
            let location = self.extract_location(input);
    ...
    fn extract_location_from_context(&self, input: &str, error_pos: usize) -> Option<String> {
        ...
        self.extract_location(context).or_else(|| self.extract_location(input))
    }
            ...
            if let Some(function) = caps.get(2) {
                if let Some(file) = caps.get(3) {
                    if let Some(line) = caps.get(4) {
                        frames.push(format!("{} at {}:{}", 
                            function.as_str(), file.as_str(), line.as_str()));
                    }
        ...
        for caps in VALGRIND_ERROR_RE.captures_iter(input) {
            if let (Some(error_type), Some(function), Some(file), Some(line)) = 
                ...
                errors.push(format!("{} in {} ({}:{})", 
                    error_type.as_str(), function.as_str(), file.as_str(), line.as_str()));
            }
        }

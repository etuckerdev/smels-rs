...
impl CsharpParser {
    ...
    fn parse_generic_csharp_error(&self, input: &str) -> Option<ErrorInfo> {
        let csharp_indicators = [".cs:", ".csproj", "dotnet", "msbuild", "System.", "namespace", "using System"];
        let has_csharp_content = csharp_indicators.iter().any(|indicator| input.contains(indicator));
        
        if has_csharp_content && (input.contains("error") || input.contains("exception") || input.contains("failed")) {
            let location = self.extract_location(input);
